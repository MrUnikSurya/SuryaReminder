...
// Shows a notification via the service worker registration — works on both
// desktop and Android. (`new Notification(...)` called from the page throws
// "Illegal constructor" on Android Chrome — it silently does nothing there,
// which is why the old test button worked on laptop but not on the phone.)
async function showLocalNotification(title, body) {
  ...
  return registration.showNotification(title, {
    ...
    tag: "surya-remind-test-" + Date.now(),
  });
}
...
async function sendTestNotification() {
  ...
  if (Notification.permission === "denied") {
    return alert("Notifications are blocked for this site — re-enable them in your browser/Android app settings first.");
  }
  if (Notification.permission !== "granted") {
    await registerPush();
  ...
  try {
    await showLocalNotification("🔔 Surya Remind", "Test notification — this device can display them.");
  } catch (e) {
    console.error("Test notification failed:", e);
    alert("Couldn't show a notification: " + e.message);
  }
}
...
async function registerPush() {
  try {
    ...
    onMessage(messaging, (payload) => {
      // Foreground push — the SW also handles background pushes independently.
      const { title, body } = payload.notification || {};
      showLocalNotification(title || "Surya Remind", body || "").catch(e => console.error(e));
    });
  } catch (e) {
...
function init() {
  ...
  if ("serviceWorker" in navigator) {
    // Register the single service worker early — it handles both offline
    // caching and (once permission is granted) push notifications. This
    // used to be two separate registrations (firebase-messaging-sw.js +
    // sw.js) both at scope "/", which could conflict; now there's one.
    navigator.serviceWorker.register("/firebase-messaging-sw.js").catch(e => console.warn("Service worker registration failed:", e));
  }
}
